...
export class SlimScroll implements OnInit, OnDestroy {
  ...
  private onWheel(e: MouseWheelEvent): void {
    ...
    // stop window scroll
    if (e.preventDefault && !this._releaseScroll) { e.preventDefault(); }
    if (!this._releaseScroll) { e.returnValue = false; }
  }
  ...
  private _queueHide: any;
  private _changesTracker: any;
